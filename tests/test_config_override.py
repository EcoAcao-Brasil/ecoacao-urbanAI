...
def test_sequence_length_override_nested():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        trainer = UrbanAITrainer(
            data_dir=Path(tmpdir) / "data", output_dir=Path(tmpdir) / "output", config=config
        )
...
def test_nested_config_priority():
    ...
    config = {
        ...
        "training": {"sequence_length": 8},  # Nested - should win
    }
...
def test_deep_merge_preserves_defaults():
    ...
    config = {"training": {"sequence_length": 4, "epochs": 150}}
...
def test_config_merge_overwrites_not_appends():
    ...
    config = {"model": {"hidden_dims": [32, 64, 128]}}  # Different from default
...
def test_empty_config_uses_all_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        trainer = UrbanAITrainer(
            data_dir=Path(tmpdir) / "data", output_dir=Path(tmpdir) / "output", config=None
        )
